...
export const LegoConfigurator: React.FC<LegoConfiguratorProps> = ({ plan, initialSelections, onShowSummary }) => {
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-8 w-full max-w-[85rem] mx-auto p-4 lg:p-8 min-h-[600px]">

            {/* === COLUMNA IZQUIERDA: PREVIEW === */}
            <div className="w-full lg:w-1/3 flex flex-col gap-4">
                <div className="relative bg-white rounded-3xl shadow-xl p-6 border border-gray-100 aspect-square flex items-center justify-center overflow-visible">

                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-yellow-300 text-yellow-900 text-xs font-bold px-4 py-1.5 rounded-full shadow-sm z-20">
                        Editando Figura {activeFigure}
                    </div>

                    <div className="relative w-full h-full border-[10px] border-slate-800 rounded-xl bg-blue-50 flex items-center justify-center overflow-hidden">
            ...
            <div className="w-full lg:w-2/3 flex flex-col gap-6">
                ...
                <div className="flex gap-3 p-1 bg-gray-100/50 rounded-2xl w-fit mx-auto lg:mx-0 flex-wrap">
                    {Array.from({ length: plan.maxFigures }, (_, i) => i + 1).map((figNum) => {
                        ...
                        return (
                            <button
                                ...
                                className={`relative flex items-center gap-2 px-5 py-2.5 rounded-xl transition-all font-bold text-sm ${
                                    activeFigure === figNum
                            ...
                            >
                                {/* Icono según el sexo */}
                                {figureSexo === 'male' ? (
                                    <span className="text-sm"><Mars size={24} /></span>
                                ) : figureSexo === 'female' ? (
                                    <span className="text-sm"><Venus size={24} /></span>
                                ) : (
                                    <User size={16} />
                                )}
                                Fig. {figNum}
                ...
                <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
                    ...
                    <div className="flex gap-3 justify-center">
                        ...
                        >
                            <span className="text-xl"><Mars size={24} /></span>
                            Hombre
                        </button>
                        <button
                        ...
                        >
                            <span className="text-xl"><Venus size={24} /></span>
                            Mujer
                        </button>
                    </div>
                </div>
                ...
                <div className={`bg-white rounded-3xl p-2 shadow-sm border border-gray-100 transition-opacity ${
                    !getCurrentFigureSexo() ? 'opacity-50 pointer-events-none' : ''
                }`}>
                    <div className="flex justify-between items-center overflow-x-auto no-scrollbar gap-2 px-2 py-2">
                        {CATEGORIES.map((cat) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`relative flex flex-col items-center justify-center gap-1 min-w-[80px] p-3 rounded-2xl transition-all flex-shrink-0 border-2 ${activeCategory === cat.id
                                ...
                                >
                                    ...
                                    {isCompleted && (
                                        <div className="absolute top-1 right-1 bg-green-500 text-white rounded-full p-0.5 shadow-sm animate-in zoom-in duration-200">
                                            <Check size={10} strokeWidth={4} />
                                        </div>
                                    )}
                ...
                {/* GRILLA DE OPCIONES */}
                <div className="bg-white rounded-[2rem] p-6 shadow-xl border border-gray-100 flex-grow relative min-h-[400px]">
                    ...
                    {!getCurrentFigureSexo() ? (
                        <div className="flex flex-col items-center justify-center h-full text-gray-400 min-h-[200px]">
                            ...
                            <p className="text-lg font-semibold">Selecciona el sexo primero</p>
                            <p className="text-sm mt-1">Elige si la figura será Hombre o Mujer</p>
                        </div>
                    ) : currentOptions.length > 0 ? (
                        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 overflow-y-auto max-h-[400px] pr-2 custom-scrollbar pb-20">
                            {currentOptions.map((item) => {
                                ...
                                <button
                                    ...
                                    disabled={isDisabled}
                                    className={`group relative aspect-square rounded-2xl bg-gray-50 border-2 transition-all overflow-hidden ${
                                        selected
                                ...
                                >
                                    {item.image ? (
                                        ...
                                        <div className="w-full h-full flex items-center justify-center bg-gray-100 text-xs text-gray-400">Sin img</div>
                                    )}

                                    <span className="absolute bottom-0 left-0 w-full text-center text-[10px] font-bold text-gray-500 bg-white/90 py-1">
                                        {item.name}
                                    </span>
                    ...
                    <div className="absolute bottom-6 right-6 z-10">
                        <button
                            ...
                            className={`flex items-center gap-2 font-bold py-3 px-6 rounded-2xl shadow-lg transition-all transform ${areAllFiguresComplete()
